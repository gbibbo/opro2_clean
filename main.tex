...
Large audio–language models (LALMs) such as Qwen2-Audio and Qwen3-Omni are widely used for high-level audio understanding, but their suitability for low-level detection under degraded conditions is unclear. We ask: (i) how well a LALM can act as a voice activity detector (VAD) under controlled acoustic degradations, and (ii) how much its behaviour can be improved by prompt optimization versus parameter-efficient fine-tuning when such adaptation is feasible. We propose a psychometric-style framework that starts from balanced speech/non-speech clips and generates 22 one-dimensional conditions across four axes: segment duration (20–1000 ms), signal-to-noise ratio, reverberation, and spectral filtering. Our primary experiments use Qwen2-Audio-7B-Instruct in a 2×2 design (base vs. LoRA; hand-crafted vs. OPRO-optimized prompts). In addition, we evaluate Qwen3-Omni-30B-A3B-Instruct as a frozen model (no LoRA/QLoRA) under hand-crafted and OPRO prompts to test whether degradation trends and prompt gains generalize to a newer omni-modal MoE model. Results show that OPRO consistently improves over a zero-shot baseline and that OPRO+LoRA yields high, stable accuracy across conditions for Qwen2-Audio, with short durations being the most challenging regime. The shortest window at which accuracy stabilizes acts as a useful proxy for the model's temporal integration scale. Although instantiated for speech vs. non-speech, the framework is class-agnostic and directly applicable to other sound events, offering a general methodology to probe and shape LALM behaviour under controlled acoustic degradations.
...
We use Qwen2-Audio~\cite{chu2024qwen2} as our primary controllable LALM; while such models excel at high-level audio understanding, their behaviour as low-level detectors under degraded conditions is unknown. Moreover, LALMs exhibit known temporal limitations~\cite{wang2025timeaudio,sridhar-etal-2025-enhancing} that may constrain fine-grained detection. In addition, we include Qwen3-Omni-30B-A3B-Instruct as a newer omni-modal Mixture-of-Experts model to test whether degradation trends and prompt-optimization gains generalize beyond Qwen2-Audio. Because parameter-efficient fine-tuning is not practical in our pipeline for Qwen3-Omni, we evaluate it as a frozen model (no LoRA/QLoRA) and focus on prompt-based adaptation.
...
\begin{enumerate}
    ...
    \item Extended model comparison: we additionally evaluate Qwen3-Omni-30B-A3B-Instruct as a frozen model (no LoRA/QLoRA) under the same psychometric degradation protocol to test whether robustness trends and prompt gains replicate on a newer omni-modal MoE LALM.
\end{enumerate}
...
The remainder of this paper is organized as follows. Section~\ref{sec:background} reviews prior work on robust VAD/SED, psychoacoustic evidence for acoustic degradations, audio-language models, and prompt optimization. Section~\ref{sec:method} describes our psychometric-style evaluation framework and optimization setup. Section~\ref{sec:results} presents experimental results.
...
We also include Qwen3-Omni-30B-A3B-Instruct as an additional evaluation model. Qwen3-Omni is an end-to-end omni-modal model with a Mixture-of-Experts (MoE) design, intended for unified text/audio interaction and low-latency speech generation.\footnote{\url{https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct} \; (model card).} The "A3B" suffix denotes that only a few billion parameters are activated per token in the MoE routing (e.g., Qwen3-30B-A3B reports 30.5B total parameters and 3.3B activated).\footnote{\url{https://huggingface.co/Qwen/Qwen3-30B-A3B-Base} \; (model overview, total vs.\ activated parameters).} We use this model to probe whether the degradation sensitivities observed for Qwen2-Audio persist in a newer omni-modal MoE LALM under a frozen (no fine-tuning) setting.
...
We present a framework to evaluate and improve Large Audio-Language Model (LALM) performance on voice activity detection under controlled acoustic degradations. The pipeline proceeds as follows: (i) extract base clips of 1000~ms from source datasets, each labeled as SPEECH or NONSPEECH; (ii) apply a bank of 22 psychometric degradation conditions spanning segment duration, signal-to-noise ratio (SNR), reverberation, and spectral filtering; (iii) embed each degraded segment in a fixed 2000~ms container padded with low-amplitude noise; (iv) query the target LALM (Qwen2-Audio or Qwen3-Omni) with a VAD prompt asking whether the clip contains human speech; and (v) normalize the model's textual response to a SPEECH/NONSPEECH label for metric computation.

This evaluation adopts a psychometric-style approach, treating accuracy as a function of each degradation parameter. By sweeping duration from 20 to 1000~ms while fixing other factors at neutral values, we trace an accuracy-versus-duration curve analogous to classical temporal integration functions. Similarly, varying SNR from $-10$ to $+20$~dB yields robustness curves. The primary Qwen2-Audio experiments follow a $2 \times 2$ factorial design crossing model type (Base versus LoRA-adapted) with prompt type (hand-crafted versus OPRO-optimized), enabling systematic comparison of these adaptation strategies. Additionally, we evaluate a frozen Qwen3-Omni-30B model (no LoRA) under hand-crafted and OPRO-optimized prompts to test cross-model generalization.
...
We apply LoRA/QLoRA only to Qwen2-Audio-7B-Instruct in this work. We do not apply LoRA/QLoRA to Qwen3-Omni-30B-A3B-Instruct because PEFT training is not practical within our current pipeline and compute budget for this omni-modal MoE stack; therefore, Qwen3-Omni is evaluated as a frozen model and we restrict adaptation to prompt optimization. As a reference point, the Qwen3-Omni model card reports substantially higher BF16 memory requirements for inference than the 7B setting.\footnote{\url{https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct} \; (see "Minimum GPU memory requirements").}

\subsection{Experimental Design}
\label{subsec:experimental_design}

The core experimental conditions for Qwen2-Audio result from crossing model type with prompt type in a $2 \times 2$ factorial design:
...
In addition to the core 2×2 design above, we include two frozen-model conditions for Qwen3-Omni-30B-A3B-Instruct (no LoRA/QLoRA) to test cross-model generalization under the same degradation protocol:
\begin{itemize}[leftmargin=*]
\item \textbf{Qwen3 (Frozen) + Hand}: Qwen3-Omni-30B-A3B-Instruct with a hand-crafted baseline prompt.
\item \textbf{Qwen3 (Frozen) + OPRO}: Qwen3-Omni-30B-A3B-Instruct with an OPRO-optimized prompt (frozen model).
\end{itemize}

All four Qwen2-Audio conditions are evaluated on an identical test set using the same evaluation script and inference parameters. The two Qwen3-Omni conditions use the same test set and evaluation script but are not included in the primary paired statistical comparisons (which focus on the Qwen2-Audio 2×2 design). The OPRO prompts differ between Base and LoRA because optimization is performed separately for each model, yielding prompts tailored to each model's behavior.
...
Experiments are run on a Slurm-managed GPU cluster. Qwen2-Audio experiments use NVIDIA RTX 3090 GPUs with 24~GB of memory (and occasionally compatible GPUs such as RTX A5000, selected via node constraints). The 4-bit NF4 quantization is required to fit Qwen2-Audio-7B-Instruct and the LoRA adapters within 24~GB, and LoRA fine-tuning uses gradient checkpointing to further reduce memory footprint. In contrast, Qwen3-Omni-30B-A3B-Instruct is evaluated on NVIDIA A100 80~GB GPUs without quantization as a frozen model (no LoRA/QLoRA); its resource demands are substantially higher than the 7B setting, so we restrict adaptation to prompt optimization.\footnote{\url{https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct} \; (see "Minimum GPU memory requirements").} A full evaluation of all 21,340 test samples takes approximately 2 hours on a single RTX 3090 for Qwen2-Audio.
...
\paragraph{Additional frozen model (Qwen3-Omni).} We additionally evaluate \texttt{Qwen/Qwen3-Omni-30B-A3B-Instruct} as a frozen model (no LoRA/QLoRA) to test cross-model generalization under the same degradation protocol. The model uses an MoE architecture; the "A3B" naming convention indicates that only a few billion parameters are active per inference step (e.g., Qwen3-30B-A3B reports 30.5B total parameters and 3.3B activated).\footnote{\url{https://huggingface.co/Qwen/Qwen3-30B-A3B-Base} \; (total vs.\ activated parameters).} We evaluate Qwen3-Omni without 4-bit or 8-bit quantization to avoid confounding robustness comparisons with quantization effects, and we restrict adaptation to prompt optimization in lieu of PEFT.

\subsubsection{OPRO Configuration}
\label{subsubsec:opro_config}

We employ Qwen2.5-7B-Instruct as the meta-LLM for prompt optimization. The meta-LLM is instructed to generate exactly three new prompts per iteration that are concise (10--300 characters). Generation uses temperature 0.7 and nucleus sampling with $p=0.9$. The optimization maintains the top-$k$ prompts with $k=10$ and runs for up to 30 iterations or until no improvement for 5 consecutive iterations (early stopping). Each prompt evaluation uses the complete development set (30 base clips $\times$ 22 conditions = 660 samples) with a fixed random seed for reproducibility.
...
Our evaluation protocol is based on a fixed test set of $970$ unique 1000-ms clips (``base clips''). Each base clip is deterministically expanded into $22$ evaluation variants spanning four psychometric degradation axes, yielding $21{,}340$ scored samples in total ($970 \times 22$). The $22$ variants comprise: (i) \textit{Duration} conditions at eight values (20--1000\,ms), (ii) \textit{SNR} at six levels ($-10$\,dB to $+20$\,dB), (iii) four \textit{Filter} settings (none, bandpass, highpass, lowpass), and (iv) four \textit{Reverberation} settings (one neutral ``none'' plus three discrete reverberation profiles). We refer to the $20$ non-neutral settings as ``degradation conditions''; two additional neutral references (filter\_none and reverb\_none) are retained for completeness and to anchor the per-axis curves.
...
We first summarize aggregate clip-level performance for the primary Qwen2-Audio configurations (2$\times$2 factorial: Base/LoRA $\times$ Hand/OPRO), using balanced accuracy (BA) as the main metric and reporting per-class recalls for SPEECH and NONSPEECH. All results in this section are computed on the same held-out test set (21,340 clips derived from 970 unique base clips), with uncertainty quantified via 95\% confidence intervals (CIs) as described in Section~\ref{subsec:statistics}. Preliminary cross-model comparison with the frozen Qwen3-Omni-30B model is presented in Appendix~\ref{app:qwen3_prelim}.
...
\midrule
Baseline vs Base+OPRO & -0.2406 & [-0.2574, -0.2233] & $<$1e-300 & $<$1e-300 & 0.378 & 1471 & 6605 \\
Baseline vs LoRA     & -0.2896 & [-0.3027, -0.2760] & $<$1e-300 & $<$1e-300 & 0.381 & 976  & 7155 \\
LoRA vs LoRA+OPRO    & -0.0188 & [-0.0229, -0.0150] & 6.78e-78 & 6.78e-78 & 0.024 & 60   & 462  \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\section{Cross-Model Generalization: Preliminary Qwen3-Omni Results}
\label{app:qwen3_prelim}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In addition to the core Qwen2-Audio experiments, we conducted a preliminary evaluation of Qwen3-Omni-30B-A3B-Instruct as a frozen model (no LoRA/QLoRA) to test whether the degradation trends and prompt-optimization gains observed for Qwen2-Audio generalize to a newer omni-modal Mixture-of-Experts (MoE) LALM. Table~\ref{tab:prelim_multimodel_comparison} compares aggregate performance across both models.

\begin{table}[h]
\centering
\caption{Preliminary cross-model comparison. The Qwen3-Omni OPRO result (91.1\%) is from full test-set evaluation; the Qwen3 baseline ($\sim$90.8\%) is from the 660-sample development subset and will be updated once the full baseline run completes. \textit{Important:} Qwen2-Audio numbers shown here are from an earlier pilot run with different evaluation parameters and are \textbf{not directly comparable} to the verified results in Table~\ref{tab:overall_performance} (main text); refer to that table for authoritative Qwen2-Audio metrics.}
...
\midrule
Qwen2-Audio-7B Base & Baseline & 77.7\%$^{\dagger}$ & 76.6\%$^{\dagger}$ \\
Qwen2-Audio-7B Base & OPRO     & 86.9\%$^{\dagger}$ & 87.5\%$^{\dagger}$ \\
Qwen2-Audio-7B LoRA & Baseline & 91.7\%$^{\dagger}$ & 92.1\%$^{\dagger}$ \\
Qwen2-Audio-7B LoRA & OPRO     & 93.7\%$^{\dagger}$ & 93.9\%$^{\dagger}$ \\
Qwen3-Omni-30B (Frozen) & Baseline$^{*}$ & $\sim$90.8\% & $\sim$92.8\% \\
Qwen3-Omni-30B (Frozen) & OPRO     & 91.1\% & 92.7\% \\
\bottomrule
\end{tabular}

\vspace{0.3em}
\footnotesize{$^{\dagger}$From pilot run; see Table~\ref{tab:overall_performance} for verified Qwen2-Audio results. $^{*}$Dev subset only.}
\end{table}

The frozen Qwen3-Omni model achieves performance comparable to the LoRA-adapted Qwen2-Audio without any parameter updates (91.1\% BA under OPRO vs. 93.7\% for Qwen2+LoRA+OPRO from the pilot run), suggesting that architectural or scale differences may confer robustness gains that Qwen2-Audio requires fine-tuning to achieve. However, the Qwen2-Audio numbers shown in this table are from an earlier pilot evaluation and should not be used for direct quantitative comparison; the verified Qwen2-Audio results reported in the main text (Table~\ref{tab:overall_performance}) use refined evaluation parameters and constitute the authoritative reference for the 2$\times$2 factorial experiment.

This preliminary cross-model snapshot demonstrates that (i) prompt optimization yields measurable gains even on a large frozen MoE model, and (ii) the psychometric degradation framework is transferable across LALM architectures. Future work will extend the full statistical analysis (McNemar tests, psychometric thresholds, condition-level sweeps) to Qwen3-Omni once the baseline evaluation completes.
...
Both Classic and Open variants used the same LoRA-adapted model and were evaluated on the identical test set (970 base clips $\times$ 22 conditions; 21,315 samples after excluding abstentions). The key differences were:
...
\begin{table}[ht]
\centering
\caption{Paired comparison of Classic vs.\ Open OPRO variants on the 21,315-sample test set (970 base clips $\times$ 22 conditions, excluding samples with abstentions). Discordant pairs indicate predictions where Classic and Open disagree.}
...
\midrule
Base+OPRO (Classic vs Open)  & --- & \multicolumn{4}{c}{N/A (only Classic variant evaluated)} \\
LoRA+OPRO (Classic vs Open)  & 0.0000 & 0  & 0.000 & 0 & 0  \\
\bottomrule
\end{tabular}

\vspace{1em}
\footnotesize
\textit{Note:} $n_{01}$ = Classic incorrect, Open correct; $n_{10}$ = Classic correct, Open incorrect. For LoRA+OPRO, the Classic and Open variants produce \textbf{identical predictions} on the test set (both correct: 20,227; both incorrect: 1,088), yielding zero discordant pairs.
\end{table}

\subsection{Discussion and Exclusion Rationale}

Analysis of the Classic and Open variants on the final test set reveals:

\begin{itemize}[leftmargin=*]
\item \textbf{Base+OPRO}: Only the Classic variant was evaluated on the final test set. Preliminary analysis on intermediate predictions (dev set + OPRO iterations) indicated substantial divergence between Classic and Open optimization trajectories, leading to the decision to focus on Classic for final evaluation.

\item \textbf{LoRA+OPRO}: The Classic and Open variants produce \textbf{identical predictions} on the test set (zero discordant pairs, $N=21{,}315$). This indicates that after LoRA adaptation, the final prompt structure (whether from Classic or Open initialization) yields equivalent VAD behavior on the evaluation domain.
\end{itemize}
